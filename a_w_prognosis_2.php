...
if(isset($_POST['c'])){
	...
	$c2=implode(',',$c2);
	//$q=query('SELECT * FROM prognosis where pro_cat IN ('.$c2.')');
}else{
	$sql='SELECT * FROM prognosis where pro_index=\'a\' ORDER BY pro_name ASC LIMIT 200';
         //die($sql);
         $sql=mysql_query($sql);
...
<?php
    ...
    for($i=0;$i<$count;$i++){
        echo '<a href="ajax_getindex_pro.php?index='.$str[$i].'" class="lex_call">'.$str[$i].'</a>';
    }
    echo '</div>'
?><form action="a_w_prognosis_3.php" method="post">

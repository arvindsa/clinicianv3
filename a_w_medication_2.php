...
if(isset($_POST['c'])){
	...
	$c2=implode(',',$c2);
	//$q=query('SELECT * FROM medication where med_cat IN ('.$c2.')');
}else{
	$sql='SELECT * FROM medication where med_index=\'a\' ORDER BY med_name ASC LIMIT 200';
         //die($sql);
         $sql=mysql_query($sql);
...
<?php
    ...
    for($i=0;$i<$count;$i++){
        echo '<a href="ajax_getindex_med.php?index='.$str[$i].'" class="lex_call">'.$str[$i].'</a>';
    }
    echo '</div>'
?> <form action="a_w_medication_3.php" method="post">

...
<?php
    ...
    echo '<div class="lex">';
    for($i=0;$i<$count;$i++){
        echo '<a href="a_disease_1.php?index='.$str[$i].'" class="ajax_call">'.$str[$i].'</a>';
    }
    echo '</div>' ;
    require('inc/sql.php');  
     if(isset($_GET['index'])){
         ...
         $sql='SELECT DISTINCT disease_name,disease_id FROM disease where disease_index=\''.$index.'\' ORDER BY disease_name ASC';
         //die($sql);
         $sql=mysql_query($sql);

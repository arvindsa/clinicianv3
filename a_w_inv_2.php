...
if(isset($_POST['c'])){
	...
	$c2=implode(',',$c2);
	//$q=query('SELECT * FROM inv where inv_cat IN ('.$c2.')');
}else{
	 $sql='SELECT * FROM inv where inv_index=\'a\' ORDER BY inv_name ASC LIMIT 200';
         //die($sql);
         $sql=mysql_query($sql);
...
<?php
    ...
    for($i=0;$i<$count;$i++){
        echo '<a href="ajax_getindex_inv.php?index='.$str[$i].'" class="lex_call">'.$str[$i].'</a>';
    }
    echo '</div>'
?><form action="a_w_inv_3.php" method="post">
